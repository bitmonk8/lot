...
96 active findings. 0 MUST FIX, 19 NON-CRITICAL, 77 NIT. Groups ordered by impact (NON-CRITICAL first, then NIT).
...
| 4 | Testing | lot/src/linux/namespace.rs | 37-40 | NON-CRITICAL | No test for `is_apparmor_restricted`. Regression silently disables sandboxing on AppArmor systems. |
...
| 6 | Testing | lot/src/linux/mod.rs | 280-282 | NON-CRITICAL | No test for `allow_network() == true` path. |
| 7 | Testing | lot/tests/integration.rs | entire file | NIT | No test for `allow_network(true)`. |
...
| # | Category | File | Line(s) | Severity | Description |
|---|----------|------|---------|----------|-------------|
| 8 | Testing | lot/src/unix.rs | 1009-1017 | NON-CRITICAL | `close_if_not_std_skips_standard_fds` has no assertion that fds 0/1/2 remain open. Test proves nothing. |
...
| 12 | Testing | lot/tests/integration.rs | 1281-1334 | NON-CRITICAL | macOS memory limit test returns early on `setrlimit` failure with a diagnostic print, but reports as passed (not skipped) to the test framework. |
...
| 14 | Testing | lot/src/path_util.rs | 14-36 | NON-CRITICAL | `is_descendant_or_equal` is `#[cfg(test)]` only. Production uses `is_strict_parent_of` which has only 4 tests with non-existent paths. |
| 15 | Testing | lot/src/path_util.rs | 48-68 | NON-CRITICAL | `canonicalize_existing_prefix` has no test for symlinks in the existing prefix — the function's stated purpose. |
