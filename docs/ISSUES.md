...
Deny paths silently not applied when path escaping fails. Sandboxed process retains access to paths the policy intended to block.

| # | File | Lines | Description | Severity |
|---|------|-------|-------------|----------|
| 1 | lot/src/macos/seatbelt.rs | 288-308 | `append_sbpl_rule` silently drops rules when `escape_sbpl_path` fails. For deny paths, the sandboxed process retains access to a path the policy intended to block. Security-relevant silent failure. | Critical |
| 2 | lot/src/macos/mod.rs | 106 | `canonicalize` failure silently swallowed. Delays error to child execve, producing a less actionable error message. | Medium |
