...
- Integration test helper deduplication: extracted `make_temp_dir`, `set_sandbox_env`, `platform_exec_paths` to `lot/tests/common/mod.rs`, eliminated three copies of platform-conditional exec_paths construction in integration tests
- CI now tests `lot-cli` crate: added `cargo test -p lot-cli` step to Linux, macOS, and Windows test jobs
